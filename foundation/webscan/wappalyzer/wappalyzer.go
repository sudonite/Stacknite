...
import (
	...
	"github.com/sudonite/stacknite/foundation/webscan/wappalyzer/data"
)
...
func New() (*Wappalyze, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create wappalyze instance: %w", err)
	}
...
func NewFromFile(filePath string, loadEmbedded, supersede bool) (*Wappalyze, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("newfromfile: %w", err)
	}
...
func (s *Wappalyze) loadFingerprints() error {
	...
	err := json.Unmarshal([]byte(data.GetRawFingerprints()), &fingerprintsStruct)
	if err != nil {
		return fmt.Errorf("loadFingerprints: %w", err)
	}
...
func (s *Wappalyze) loadFingerprintsFromFile(filePath string, loadEmbedded, supersede bool) error {
	...
	if loadEmbedded {
		...
		err := json.Unmarshal([]byte(data.GetRawFingerprints()), &embedded)
		if err != nil {
			return fmt.Errorf("unmarshal fingerprint: %w", err)
		}
...
func (u UniqueFingerprints) SetIfNotExists(value, version string, confidence int) {
	if _, ok := u.values[value]; ok {
		...
		updatedConfidence := min(new.confidence+confidence, 100)
...
func AppInfoFromFingerprint(fingerprint *CompiledFingerprint) AppInfo {
	...
	for _, cat := range fingerprint.cats {
		if category, ok := data.GetRawCategories()[cat]; ok {
