...
const pathFingerprints = "foundation/webscan/wappalyzer/data/fingerprints.json"
...
type OutputFingerprint struct {
	...
	DOM         map[string]map[string]any `json:"dom,omitempty"`
...
func main() {
	fingerprints := flag.String("fingerprints", pathFingerprints, "File to write wappalyzer fingerprints to")
	flag.Parse()
...
func gatherFingerprintsFromURL(URL string, fingerprints *Fingerprints) error {
	...
	maps.Copy(fingerprints.Apps, fingerprintsOld.Apps)
...
func normalizeFingerprints(fingerprints *Fingerprints) *OutputFingerprints {
	...
	for app, fingerprint := range fingerprints.Apps {
		output := OutputFingerprint{
			...
			DOM:         make(map[string]map[string]any),
		...
		if fingerprint.Dom != nil {
			...
			case reflect.String:
				...
				output.DOM[data] = map[string]any{"exists": ""}
			case reflect.Slice:
				data := v.Interface().([]any)
				for _, pattern := range data {
					pat := pattern.(string)
					output.DOM[pat] = map[string]any{"exists": ""}
				}
			case reflect.Map:
				data := v.Interface().(map[string]any)
				for pattern, value := range data {
					output.DOM[pattern] = value.(map[string]any)
				}
			}
		}
